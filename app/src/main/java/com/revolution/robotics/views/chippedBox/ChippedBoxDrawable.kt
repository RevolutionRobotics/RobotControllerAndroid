...
class ChippedBoxDrawable(context: Context, config: ChippedBoxConfig) : StateListDrawable() {
    ...
    init {
        addState(IntArray(1) { android.R.attr.state_pressed }, StateDrawable(context, true, config))
        addState(StateSet.WILD_CARD, StateDrawable(context, false, config))
    }

    private class StateDrawable(
        context: Context,
        private val isPressedState: Boolean,
        private val config: ChippedBoxConfig
    ) : Drawable() {
        ...
        private val pressedStateOverlayPaint = Paint().apply {
            style = Paint.Style.FILL
            color = context.color(R.color.overlay_white_28)
        }
        ...
        private fun Canvas.drawChippedBox(
            ...
            val path = pathData.buildPath()
            drawPath(path, contentPaint)

            if (isPressedState) {
                drawPath(path, pressedStateOverlayPaint)
            }
        }
